...
export const FullListScreen: React.FC<FullListScreenProps> = ({ onBack }) => {

  const dataByCity = FULL_DATA.reduce((acc, department) => {
    const city = department.city;
    if (!acc[city]) {
      acc[city] = [];
    }
    acc[city].push(department);
    return acc;
  }, {} as Record<string, DepartmentData[]>);

  const cities = Object.keys(dataByCity).sort((a,b) => a.localeCompare(b, 'ku'));
    ...
    <div className="w-full max-w-7xl mx-auto animate-fade-in">
      <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
        <h1 className="text-3xl font-bold text-slate-800 lang-ku" dir="rtl">
          لیستا گشتی یا کونمرێن وەرگرتنێ (٢٠٢٤-٢٠٢٥)
        </h1>
        <button
          onClick={onBack}
              ...
              <table className="w-full text-right border-collapse lang-ku" dir="rtl">
                <thead>
                  <tr className="bg-slate-100">
                    ...
                    <th className="p-4 font-semibold text-slate-600 text-center">ئێڤاران</th>
                  </tr>
                </thead>
